...
public class ConfigHandler {
    ...
    private static void setConfigFile() {
        ...
        // config.yml
        fileMap = new FileMap();
        filePath = SlimeChunkPlus.getInstance().getDataFolder().getPath();
        fileName = "config.yml";
    ...
    private static void checkResource(File file, String resource) {
        if (!(file).exists()) {
            try {
                SlimeChunkPlus.getInstance().saveResource(resource, false);
            } catch (Exception e) {
    ...
    private static void checkConfigVer(String fileName) {
        ...
        if (file.exists() && getConfig(fileName).getInt("Config-Version") != version) {
            if (SlimeChunkPlus.getInstance().getResource(fileName) != null) {
