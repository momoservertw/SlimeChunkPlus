...
public class Logger {
    private static final File defaultFile = new File(SlimeChunkPlus.getInstance().getDataFolder().getPath() + "\\Logs\\latest.log");
    ...
    public void createDefaultLog() {
        File folder = new File(SlimeChunkPlus.getInstance().getDataFolder().getPath() + "\\Logs");
        // Check log folder.
        if (!folder.exists()) {
            try {
                if (!folder.mkdir()) {
